  ...
  return (
    <>
      ...
      <div className={`${styles.searchBox} py-4`}>
        ...
        <input className={styles.customInput} type="text" list="locations" name="location" id="location" placeholder={`${t("Location")}`} ref={location} onChange={searchBloodGroupLocation} />
      ...
      <table className={`${styles.tableW} table w-75 mt-5 mx-auto`}>
        ...
        <tbody>
          {searchRes!==null && searchRes.length === 0 ? <tr><td colSpan={3} className="fs-4">{t("Sorry, no results found")}</td></tr>: searchRes?searchRes.slice(startIndex, endIndex).map(vol => !vol.isVolunteer&&<tr key={uuid()}>
      ...
      <button className="btn btn-outline-danger d-block mx-auto fw-bold my-2 mb-5">{t("Start saving lives")}</button>
    </>
  );
};
